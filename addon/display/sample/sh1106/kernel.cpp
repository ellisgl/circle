...
const CI2CMaster I2C_MASTER_DEVICE = (CMachineInfo::Get ()->GetDevice (DeviceI2CMaster));
...
TShutdownMode CKernel::Run (void)
{
    ...
    for (unsigned nCount = 0; 1; nCount++)
    {
        ...
        cPos = (nCount > 128/8) ? nCount - 128/8 : nCount;
        if (nResult > 0) {
            m_LCD.PrintText (Buffer, cPos, 0);
        }
...
void CKernel::LCDWrite (const char *pString, int x, int y)
{
    m_LCD.PrintText (pString, x, y);
}
